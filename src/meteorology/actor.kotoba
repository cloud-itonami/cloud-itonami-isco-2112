(ns meteorology.actor
  "MeteorologicalActor — the ISCO-08 2112 meteorology support actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one weather operation request
  ...
  The unconditional invariant: the MeteorologicalAdvisor can never
  directly commit a record or dispatch a robot action the
  MeteorologicalGovernor refuses — every commit-record! call is gated
  behind `:decide`. This is especially critical for weather operations
  that affect public safety (forecasts, severe-weather flags)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [meteorology.advisor :as advisor]
            [meteorology.governor :as governor]
            [meteorology.store :as store]))

(defn build-graph
  "Build a compiled MeteorologicalActor graph. `store` implements
  `meteorology.store/Store`. `advisor` implements
  `meteorology.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:station-id (:station-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one weather operation request to completion or interrupt.
